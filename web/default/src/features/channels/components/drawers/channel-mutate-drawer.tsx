...
import {
  buildApiAccessFields,
  genericBaseUrlField,
  renderApiAccessField,
  TYPES_WITHOUT_GENERIC_BASE_URL,
} from './channel-api-access-fields'
import {
...
export function ChannelMutateDrawer({
  ...
  const onInvalid: SubmitErrorHandler<ChannelFormValues> = useCallback(
    (errors) => {
      // Auto-expand the collapsed Advanced Settings section when it holds an
      // error, so the offending field is visible before we scroll it into view.
      if (hasAdvancedSettingsErrors(errors)) {
        handleAdvancedSettingsOpenChange(true)
      }
      toast.error(t('Please fix the highlighted fields before saving'))

      // Scroll the first invalid field into view and focus it. Defer to the
      // next frame so a just-expanded collapsible has rendered its content.
  ...
  // ── Per-type API-access field escape hatches ──
  // These render the handful of provider fields that need component-local state
  // or bespoke markup the declarative registry can't express. Behavior is
  // preserved verbatim from the previous inline branches.

  // SiliconFlow (type 40) — referral link alert.
  const renderSiliconFlowAlert = () => (
    <Alert>
      <AlertDescription>
        {t('Referral link:')}{' '}
        <a
  ...
  // Vertex AI (type 41) — service account JSON file upload, now RHF-tracked via
  // FormField on `key` so its value is validated like every other field (B3).
  const renderVertexJsonFileUpload = () => {
    if (form.watch('vertex_key_type') !== 'json') return null
    return (
      <FormField
          ...
          <FormItem>
            ...
            <FormControl>
              <Input
                ...
                // The `key` form value is the source of truth; this is a
                // write-only control that parses uploads INTO `key` via
                // setValue. Wrapping it in FormField keeps the upload's
                // validation state (FormMessage / aria) bound to `key` (B3).
                // We intentionally do NOT forward `field.ref`/`value`: a file
                // input must stay uncontrolled, and the `key` textarea owns the
                // RHF ref for error focus.
                onChange={async (e) => {
  ...
  // VolcEngine (type 45) — base_url has two mutually exclusive modes gated by the
  // secret click-to-unlock state (locked: region select; unlocked: free input).
  const renderVolcEngineBaseUrl = () => {
    if (!doubaoApiEditUnlocked) {
      return (
        <FormField
          control={form.control}
          name='base_url'
  ...
  // Ordered API-access field descriptors for the current channel type, plus the
  // generic Base URL field for every type except those that own it themselves.
  const apiAccessFields = buildApiAccessFields(currentType, {
    t,
    renderSiliconFlowAlert,
    renderVolcEngineBaseUrl,
  ...
  return (
    <>
      <DrawerShell
        ...
        size='xl'
        ariaTitle={isEditing ? t('Edit Channel') : t('Create Channel')}
        ariaDescription={headerDescription}
      >
        <DrawerHeader
          title={headerTitle}
          description={headerDescription}
          icon={getLobeIcon(`${getChannelTypeIcon(currentType)}.Color`, 18)}
        />
            ...
            {isChannelDetailLoading ? (
              ...
              <>
                ...
                <ChannelBasicSection>
                  ...
                  <DrawerSwitchGroup>
                    <FormField
                      ...
                      name='status'
                      render={({ field }) => (
                        <FormItem className='flex min-h-16 flex-row items-center justify-between gap-3 px-3 py-3'>
                ...
                <ChannelApiAccessSection>
                  ...
                  {/* Provider-specific API-access fields, driven by the
                        per-type registry (see channel-api-access-fields.tsx).
                        Each descriptor renders the SAME markup the old inline
                        `currentType === N` branches did; behavior is preserved
                        exactly per channel type. */}
                  {apiAccessFields.map((descriptor, index) => {
                    const fieldKey =
                      descriptor.kind === 'custom'
                        ? descriptor.key
                        : `${descriptor.kind}-${descriptor.name}-${index}`
                    return (
        ...
        <DrawerFooter
          ...
          submitLabel={isEditing ? t('Update Channel') : t('Save changes')}
          submittingLabel={isEditing ? t('Update Channel') : t('Save changes')}
