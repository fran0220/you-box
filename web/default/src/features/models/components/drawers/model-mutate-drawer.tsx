...
export function ModelMutateDrawer({
  ...
  // Load model data for editing and ratio configuration. form.reset mutates
  // the form store, so this stays in an effect; the work is deferred to a
  // cancellable microtask to avoid synchronous effect setState.
  useEffect(() => {
    let cancelled = false
    queueMicrotask(() => {
      if (cancelled) return
