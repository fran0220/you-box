...
export function SubscriptionPlansCard({
  ...
  return (
    <>
      <Panel>
        <PanelHeader title={t('Subscription Plans')} />
        <PanelBody className='space-y-4 sm:space-y-5'>
              ...
              {plans.map((p, index) => {
                ...
                return (
                  <PlanCard
                    ...
                    price={`$${price}`}
                    unit={`/ ${formatDuration(plan, t)}`}
                    description={[plan.subtitle, ...benefits]
                      .filter(Boolean)
                      .join(' · ')}
                    current={isCurrent && reached}
                    action={
                      reached ? (
                        <Tooltip>
