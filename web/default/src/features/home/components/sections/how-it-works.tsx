  ...
  if (props.guide.to.startsWith('http')) {
    return (
      <a
        ...
        className={className}
        data-slot={props.circuit ? 'circuit-guide' : undefined}
      >
        {props.children}
      </a>
    )
...
/**
 * Entry points. Circuit: elevated guide cards. Paper: hairline columns.
 */
export function HowItWorks() {
  ...
  return (
    <section
      ...
      className={cn(
        'relative z-10',
        !isCircuit && 'border-border/70 border-t py-14 md:py-20'
      )}
    >
      <div
        className={cn(
            ...
            {guides.map((guide, i) => (
              ...
              >
                <GuideLink guide={guide} circuit>
                  ...
                  <p className='text-muted-foreground max-w-[30em] flex-1 text-sm leading-relaxed'>
                    {guide.desc}
                  </p>
                  <span className='text-brand mt-auto inline-flex items-center gap-1.5 pt-1 text-sm font-medium'>
                    {guide.cta}
                    <ArrowRight
