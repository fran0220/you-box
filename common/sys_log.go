...
func LogStartupSuccess(startTime time.Time, port string) {
	...
	_, _ = fmt.Fprintf(gin.DefaultWriter, "  \033[32m%s %s\033[0m  ready in %d ms\n", SystemName, Version, durationMs)
	_, _ = fmt.Fprintf(gin.DefaultWriter, "\n")

	if !IsRunningInContainer() {
		_, _ = fmt.Fprintf(gin.DefaultWriter, "  ➜  \033[1mLocal:\033[0m   http://localhost:%s/\n", port)
	}

	for _, ip := range networkIps {
		_, _ = fmt.Fprintf(gin.DefaultWriter, "  ➜  \033[1mNetwork:\033[0m http://%s:%s/\n", ip, port)
	}
